/**
 * 依存ライブラリなしの純粋なJavaScript実装によるMD5ハッシュ計算。
 * Web Crypto API（SubtleCrypto）はセキュリティ上の理由からMD5を提供していないため、
 * RFC 1321に基づく実装をここに用意する。
 */
...
function md5Core(x: number[], len: number): [number, number, number, number] {
  x[len >> 5] |= 0x80 << (len % 32);
  x[(((len + 64) >>> 9) << 4) + 14] = len;
